rules = [ // bags contain no other bags.
...
for(let i = 0; i < rules.length; i++) {
	arrayRule = rules[i].split(" ");

	key = arrayRule[0] + " " + arrayRule[1];

	for(let j = 3; j < arrayRule.length; j++) {
		color[key] += " " + arrayRule[j];
	}
}

function f(color, key) {
	console.log(color[key])
	if(color[key] == " contain no other bags.") return 0;

	digits = ["1","2","3","4","5","6","7","8","9"];
	ruleWord = color[key].split(" ");
	cases = [];

	for(let i = 0; i < ruleWord.length; i++) {
		if(digits.includes(ruleWord[i]))
			cases.push([parseInt(ruleWord[i]), ruleWord[i + 1] + " " + ruleWord[i + 2]])
	}
	...
	for(currentCase of cases) {
		...
		result += currentCase[0] + currentCase[0] * f(color, currentCase[1]);
	}
...
f(color, "shiny gold")
console.log(f(color, "shiny gold"))