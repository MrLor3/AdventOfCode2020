...
bagsColorContainingShinyGold = [];

for(key in color) {
	if(color[key].includes("shiny gold")) {
		bagsColorContainingShinyGold.push(key);
	}
}

for(bagColor of bagsColorContainingShinyGold) {
	for(key in color) {
		if(color[key].includes(bagColor)) {
			bagsColorContainingShinyGold.push(key);
		}
	}
}
...
bagsColor = arrayUnique(bagsColorContainingShinyGold);

console.log(bagsColor.length)
